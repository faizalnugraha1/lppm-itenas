                    ...
                    <tr>
                        ...
                        <td class="border-b w-5">
                            <div class="flex sm:justify-center items-center">
                                <button class="show-data button px-2 mr-1 mb-2 bg-theme-14 text-theme-10 tooltip" data-url="@if($d->table_name == 'Penelitian') {{ route('penelitian.show', [$d->id]) }} @elseif ($d->table_name == 'Pkm') {{ route('pkm.show', [$d->id]) }} @elseif($d->table_name == 'Insentif') {{ route('insentif.show', [$d->id]) }} @elseif($d->table_name == 'HKI') {{ route('hki.show', [$d->id]) }} @elseif($d->table_name == 'Publikasi') {{ route('publikasi.show', [$d->id]) }} @endif" title="Lihat"> <span class="w-5 h-5 flex items-center justify-center"> <i data-feather="eye" class="w-4 h-4"></i> </span> </button>
                                <a href="@if($d->table_name == 'Penelitian') {{ route('acc.penelitian', [$d->id]) }} @elseif ($d->table_name == 'Pkm') {{ route('acc.pkm', [$d->id]) }} @elseif($d->table_name == 'Insentif') {{ route('acc.insentif', [$d->id]) }} @elseif($d->table_name == 'HKI') {{ route('acc.hki', [$d->id]) }} @elseif($d->table_name == 'Publikasi') {{ route('acc.publikasi', [$d->id]) }}@endif" class="button btn-edit px-2 mr-1 mb-2 bg-theme-18 text-theme-9 tooltip" title="Terima"> <span class="w-5 h-5 flex items-center justify-center"> <i data-feather="check-square" class="w-4 h-4"></i> </span> </a>
                                <a href="@if($d->table_name == 'Penelitian') {{ route('penelitian.edit', [$d->id]) }} @elseif ($d->table_name == 'Pkm') {{ route('pkm.edit', [$d->id]) }} @elseif($d->table_name == 'Insentif') {{ route('insentif.edit', [$d->id]) }} @elseif($d->table_name == 'HKI') {{ route('hki.edit', [$d->id]) }} @elseif($d->table_name == 'Publikasi') {{ route('publikasi.edit', [$d->id]) }}@endif" class="button btn-edit px-2 mr-1 mb-2 bg-theme-17 text-theme-11 tooltip" title="Edit"> <span class="w-5 h-5 flex items-center justify-center"> <i data-feather="edit-3" class="w-4 h-4"></i> </span> </a>
                                <a href="@if($d->table_name == 'Penelitian') {{ route('del.penelitian', [$d->id]) }} @elseif ($d->table_name == 'Pkm') {{ route('del.pkm', [$d->id]) }} @elseif($d->table_name == 'Insentif') {{ route('del.insentif', [$d->id]) }} @elseif($d->table_name == 'HKI') {{ route('del.hki', [$d->id]) }} @elseif($d->table_name == 'Publikasi') {{ route('del.publikasi', [$d->id]) }}@endif" class="button btn-edit px-2 mr-1 mb-2 bg-theme-31 text-theme-6 tooltip" title="Hapus"> <span class="w-5 h-5 flex items-center justify-center"> <i data-feather="trash-2" class="w-4 h-4"></i> </span> </a>
                            </div>
                        </td>
                    </tr>
                    @endforeach
